...
namespace MUtils
{
	...
	{
		std::vector<char> v(in_str.begin(), in_str.end());

		return encode(v);
	}

	std::string Base64Helper::encode(const char* buffer, size_t length)
	{
		std::vector<char> v(buffer, buffer + length);

		return encode(v);
	}

	std::string Base64Helper::encode(const std::vector<char>& in_str)
	{
		std::string out_str;
	...
	std::vector<char> Base64Helper::decode(const std::string& in_str)
	{
		...
		while ( i<len)
		{
			...
			out_str.push_back(char( (c1<<2) | ((c2&0x30)>>4) ));
			...
			out_str.push_back(char( ((c2&0XF)<<4) | ((c3&0x3C)>>2) ));
			...
			out_str.push_back(char( ((c3&0x03)<<6) | c4 ));
		}
